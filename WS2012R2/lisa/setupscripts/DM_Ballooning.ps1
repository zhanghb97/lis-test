...
# Test ballooning by using stress-ng for a second time

# Sleep 10 minutes to make the memory balloon
Start-Sleep -s 600
...
"Memory stats after $vm1Name ballooning."
"  ${vm1Name}: assigned - $vm1BeforeAssigned | demand - $vm1BeforeDemand"
...
# Calculate the amount of memory to be consumed on VM1 with stress-ng
[int64]$vm1ConsumeMem = (Get-VMMemory -VM $vm1).Minimum
...
"Memory stats after $vm1Name started stress-ng for the second time."
"  ${vm1Name}: assigned - $vm1Assigned | demand - $vm1Demand"

if ($vm1Assigned -le $vm1BeforeAssigned)
{
    "Error: Memory Demand did not increase after starting stress-ng. (After ballooning)" | Tee-Object -Append -file $summaryLog
    return $false
}
...
if ($vm1AfterAssigned -ge $vm1Assigned)
{
    "Error: Demand did not go down after stress-ng finished.(After ballooning)" | Tee-Object -Append -file $summaryLog
    return $false
}

"Memory Ballooning (using stress-ng) completed successfully!" | Tee-Object -Append -file $summaryLog
return $true
